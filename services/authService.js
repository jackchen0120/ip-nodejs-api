...
const setAuthUser = (user, type, accessToken, status, loginTimes) => {
	let sql = '';
	if (status == 0) {
		sql = `update user_third_auth set type='${type}', username='${user.login}', nickname='${user.name}', 
		access_token='${accessToken}', avatar_url='${user.avatar_url}', login_time='${moment().format('YYYY-MM-DD HH:mm:ss')}', login_times='${loginTimes}' where openid='${user.id}'`;
	} else if (status == 1) {
		sql = `insert into user_third_auth(openid, type, username, nickname, access_token, avatar_url, create_time, login_time, login_times) 
		values('${user.id}', '${type}', '${user.login}', '${user.name}', '${accessToken}', '${user.avatar_url}', '${moment().format('YYYY-MM-DD HH:mm:ss')}', '${moment().format('YYYY-MM-DD HH:mm:ss')}', 1)`;
	}
	return queryOne(sql);
}
